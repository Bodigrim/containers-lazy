-- Since 'Data.Set.Lazy' follows API of 'Data.Set',
-- this file was copy-pasted from 'containers' package.
...
test_lookupLT = do
    lookupLT (3 :: Integer) (fromList [3, 5]) @?= Nothing
    lookupLT (5 :: Integer) (fromList [3, 5]) @?= Just 3
...
instance Arbitrary TwoSets where
  arbitrary = do
    ...
    pure (TwoSets (fromList l) (fromList r))
...
prop_foldR s = Foldable.foldr (:) [] s == toList s

prop_foldR' :: Set Int -> Bool
prop_foldR' s = Foldable.foldr' (:) [] s == toList s

prop_foldL :: Set Int -> Bool
prop_foldL s = Foldable.foldl (flip (:)) [] s == List.foldl (flip (:)) [] (toList s)
...
prop_partition s _ = case partition odd s of
...
valid s = s' == s''
  where
    s'  = toAscList s
    s'' = List.map head (List.group (sort s'))
